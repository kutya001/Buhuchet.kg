  ...
  const handleSave = async (status: 'draft' | 'sent' = 'sent') => {
    ...
    startTransition(async () => {
      ...
      if (editDocId) {
        ...
        if (res.success) {
          router.push(`/uchet/documents/${editDocId}`);
        } else {
        ...
        if (res.success && res.data) {
          router.push(`/uchet/documents/${res.data.id}`);
        } else {
            ...
            <div className="space-y-2">
              ...
              {partners.length === 0 ? (
                <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20 text-xs text-amber-500 flex items-center justify-between">
                  ...
                  <Link href="/uchet/counterparties" className="underline font-bold">
                    Модуль Контрагенты
                  </Link>
                </div>
              ) : (
