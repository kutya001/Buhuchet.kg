-- =============================================================================
-- МИГРАЦИЯ 20260809120000: ОПТИМИЗАЦИЯ RLS ПОЛИТИК И ПРОВЕРКА ЗАКРЫТЫХ ПЕРИОДОВ
-- =============================================================================

-- 1. Мемоизация выражения (SELECT auth.uid()) в RLS-политиках для исключения ре-эвалюации на каждую строку
ALTER TABLE public.documents ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.files ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.file_owners ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.company_partnerships ENABLE ROW LEVEL SECURITY;

-- 1.1 Политика для documents
...
-- 1.3 Политика для file_owners
DROP POLICY IF EXISTS "Доступ к владельцам файлов своей компании" ON public.file_owners;
CREATE POLICY "Доступ к владельцам файлов своей компании" ON public.file_owners
FOR ALL USING (
  company_id IN (SELECT company_id FROM public.users WHERE id = (SELECT auth.uid()))
);

-- 1.4 Политика для company_partnerships
...
-- 2. Актуализация триггерной функции public.check_closed_period_lock()
...
DECLARE
    ...
    v_closed_until DATE;
BEGIN
    -- Получаем роль и статус суперадмина текущего пользователя с мемоизированным (SELECT auth.uid())
    ...
    -- Проверка 1: Журнал закрытых периодов (company_closed_periods)
    SELECT EXISTS (
    ...
    -- Проверка 2: Порог закрытия в профиле организации (companies.closed_period_until)
...
-- Убедимся, что триггеры подключены на INSERT, UPDATE, DELETE для documents и files
