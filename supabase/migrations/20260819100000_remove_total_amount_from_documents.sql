-- ==============================================================================
-- МИГРАЦИЯ 20260819100000: Удаление колонки total_amount из documents
-- ==============================================================================

-- 1. Удаление колонки total_amount из таблицы documents
ALTER TABLE public.documents DROP COLUMN IF EXISTS total_amount;

-- 2. Обновление процедуры create_document_atomic
    ...
    IF p_files IS NOT NULL AND jsonb_array_length(p_files) > 0 THEN
        ...
        LOOP
            INSERT INTO public.files (
                ...
                p_company_id,
                v_doc_id,
                CASE WHEN (v_file->>'category_id') IS NOT NULL AND (v_file->>'category_id') != '' THEN (v_file->>'category_id')::UUID ELSE NULL END,
                v_file->>'file_name',
...
-- 3. Перезагрузка схемы PostgREST
