...
} from '@/app/uchet/subscription/actions';
...
import type { SubscriptionRenewalRequest } from '@/types/database.types';
            ...
            <div className="flex items-baseline justify-between">
              <span
                className={`text-base font-extrabold ${
                  limits?.isTelegramEnabled ? 'text-emerald-400' : 'text-muted-foreground'
                }`}
              >
                {limits?.isTelegramEnabled ? 'Подключен' : 'Отключен'}
              </span>
              <span className="text-xs text-muted-foreground">
                {limits?.isTelegramEnabled ? 'Активен' : 'Недоступен'}
              </span>
            </div>

            <p className="text-[11px] text-muted-foreground">
              {limits?.isTelegramEnabled
                ? 'Быстрые оповещения включены'
                : 'Доступно в Бизнес и Премиум'}
            </p>
          </Card>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Выберите оптимальный тариф для масштабирования документооборота компании
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {plans.map((plan) => {
            ...
            const storageGb = plan.storage_limit_bytes
              ? Math.round(plan.storage_limit_bytes / (1024 * 1024 * 1024))
              : 1;
            ...
            return (
              ...
              >
                ...
                <div className="pt-6 mt-4 border-t border-border">
                  ...
                  >
                    {isCurrent ? 'Продлить этот тариф' : `Перейти на «${plan.name}»`}
                  </Button>
                </div>
              </Card>
            );
      ...
      <Card className="bg-card border-border p-5 space-y-4 shadow-xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Статусы обработки ваших запросов администратором сервиса
            </p>
          </div>
