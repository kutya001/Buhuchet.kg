...
export function UnifiedFormModal({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-200">
      <Card className="w-full sm:max-w-2xl bg-slate-900 border-t sm:border border-slate-800 rounded-t-3xl sm:rounded-2xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
        {/* Мобильная полоса перетягивания */}
        <div className="w-12 h-1 bg-slate-700 rounded-full mx-auto mt-2 mb-1 sm:hidden opacity-80" />
        ...
        <CardHeader className="p-4 md:p-6 border-b border-slate-800 flex flex-row items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <CardTitle className="text-base md:text-lg font-bold text-white flex items-center space-x-2">
                <span>{title}</span>
              </CardTitle>
          ...
          <div className="flex items-center space-x-2">
            {mode === 'view' && onSwitchMode && (
              <Button
                ...
                className="border-slate-800 text-amber-400 text-xs min-h-[36px]"
              >
