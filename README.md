...
- 👥 **Разделение Регистрации, Гостевой Режим и Заявки в Штат:**
  Два независимых сценария онбординга:
  - 👑 **Владелец компании**: Создание организации и реквизитов на шаге `/onboarding`.
  - 👤 **Сотрудник**: Мгновенная регистрация без юрлица, защищенный гостевой режим (`/dashboard/pending`), поиск организаций по ИНН/названию, подача заявок в штат (`company_join_requests`) и мгновенные Telegram-оповещения руководству.
- 💼 **Управление Персоналом и Ролями RBAC (`/dashboard/employees`):**
  Параллельная загрузка (`Promise.all`), реестр активных сотрудников, утверждение заявок соискателей с назначением должности и роли, гибкая матрица доступов по модулям и Telegram-уведомления соискателям.
...
- 🤖 **Telegram-Бот Интеграция:**
  Уведомления о новых документах, заявках соискателей и верификации через Telegram Webhooks и Upstash QStash.
