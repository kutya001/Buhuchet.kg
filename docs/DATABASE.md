...
| `20260822000000_subscription_limits_and_renewal_requests.sql` | 22.08.2026 00:00 | Таблица `subscription_renewal_requests`, квоты тарифов (`max_counterparties`, `max_employees`, `storage_limit_bytes`, `is_telegram_enabled`), оверрайды в `companies`, хранимая процедура `admin_approve_renewal_request_atomic` |
| `20260823000000_storage_atomic_rpc_and_counterparty_limits.sql` | 23.08.2026 00:00 | Столбец `storage_used_bytes` в `companies`, хранимая функция `increment_company_storage`, индексы двунаправленного подсчета контрагентов `idx_partnerships_bidirectional` и уточненный расчет пролонгации подписок |