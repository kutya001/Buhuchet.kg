...
- `/register` — Регистрация в платформе с разделением на два независимых сценария:
  ...
  - 👤 **Сотрудник компании**: Быстрая регистрация (ФИО, E-mail, Телефон, Пароль) с автоматическим переводом в защищенный гостевой режим (`/dashboard/pending`).
...
- `/dashboard/pending` — **Гостевой Режим / Кабинет Соискателя**: Полнотекстовый поиск компаний КР по ИНН и названию, подача заявок в штат (`company_join_requests`), отслеживание статуса рассмотрения и отзыв заявок.
...
- `/dashboard/employees` — Управление персоналом, зачисление кандидатов из заявок (`pendingRequests`) и матрица ролей RBAC.
...
- `/super-admin` — **Сводный Дашборд Платформы**: Мониторинг ключевых метрик (Организации, Пользователи, Документооборот, Объем R2, Подписки) с серверным кэшированием `unstable_cache` (TTL 60s) и быстрыми переходами.
- `/super-admin/companies` — Реестр организаций, атомарная модерация (`admin_approve_company_atomic`), верификация, блокировка и детальный профиль тенантов.
- `/super-admin/users` — Реестр пользователей, сброс паролей, администрирование системных ролей и аудит авторизаций.
- `/super-admin/files` — Мониторинг Облачного диска R2, CoW дедупликация и кнопка пакетной очистки очереди `processStorageCleanupQueueAction`.
- `/super-admin/subscriptions` — Реестр и управление тарифными планами, лимитами объема первички и запуск проверки просроченных подписок (`cron_check_expired_subscriptions`).
- `/super-admin/telegram` — Центр Telegram: регистрация вебхука с `secret_token`, логирование сообщений и пакетная рассылка с контролем rate-limit.
